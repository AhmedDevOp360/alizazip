...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        if ($is_suspend) {
             // Find the latest suspended invoice number
            $last_suspended = Transaction::where('invoice_no', 'like', 'SP-%')
            ->where('business_id', $business_id)
                ->orderByDesc('id')
                ->first();
            $next_number = 1;
            if ($last_suspended && preg_match('/SP-(\d+)/', $last_suspended->invoice_no, $matches)) {
                $next_number = intval($matches[1]) + 1;
            }
            $invoice_no = [
                'invoice_no' => 'SP-' . str_pad($next_number, 5, '0', STR_PAD_LEFT),
                'prefix' => '',
                'count' => '',
                'resolution' => ''
            ];
        ...
        if (empty($pay_term_number) || empty($pay_term_type)) {
          ...
          // Handle $invoice_total as array or float
        $total_before_tax = is_array($invoice_total) && isset($invoice_total['total_before_tax']) ? $invoice_total['total_before_tax'] : 0;
        $tax_amount = is_array($invoice_total) && isset($invoice_total['tax']) ? $invoice_total['tax'] : 0;
    ...
    public function deleteSellLines($transaction_line_ids, $location_id, $adjust_qty = true)
    {
        if (! empty($transaction_line_ids)) {
            ...
            foreach ($sell_lines as $line) {
                if ($adjust_qty) {
                    $this->adjustQuantity($line->location_id, $line->product_id, $line->variation_id, $line->quantity);
                }
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        {
            $output['delivery_person_label'] = 'Delivery Person';
            $output['delivery_person'] = !empty($transaction->delivery_person_user) ? $transaction->delivery_person_user->first_name.' '.$transaction->delivery_person_user->last_name  : '';
        }
        ...
        foreach ($lines as $line) {
            ...
            if ($sub_unit_id && $product && is_array($product->sub_unit_prices)) {
                $has_subunit_price = array_key_exists($sub_unit_id, $product->sub_unit_prices)
                    && $product->sub_unit_prices[$sub_unit_id] !== null
                    && $product->sub_unit_prices[$sub_unit_id] !== '';
            }
    ...
    public function addSellReturn($input, $business_id, $user_id, $uf_number = true,$invoice_scheme_id)
    {
        ...
        foreach ($sell->sell_lines as $sell_line) {
            if (array_key_exists($sell_line->id, $returns)) {
                ...
                $line_location_id = $sell_line->location_id ?? $sell->location_id;
                $productUtil->updateProductQuantity($line_location_id, $sell_line->product_id, $sell_line->variation_id, $quantity, $quantity_before, null, false);
              }
        }
    ...
    public function registerReport($business_id, $permitted_locations, $start_date = null, $end_date = null, $user_id = null)
    {
        $registers = CashRegister::leftjoin(
            ...
            // Exclude general (Non-POS) closings from cash closing report
            ->where(function ($q) {
