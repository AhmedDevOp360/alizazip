...
class ReportController extends Controller
{
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        if ($request->ajax()) {
           ...
           if(!empty($request->get('invoice_scheme_id'))){
                if($request->invoice_scheme_id !== 'suspended'){
                    $query->where('t.invoice_scheme_id', $request->get('invoice_scheme_id'))->where('t.is_suspend', 0);
                }else{
                    $query->where('t.is_suspend', 1);
                }
            }else{
                $query->where('t.is_suspend', 0);
            }
        ...
        $brands = Brands::forDropdown($business_id);
        $customer_group = CustomerGroup::forDropdown($business_id, false, true);
        $invoice_schemes = $invoice_schemes->prepend(__('lang_v1.suspend'), 'suspended')->sort();
        
        return view('report.product_sell_report')
            ->with(compact('business_locations', 'customers', 'categories', 'brands',
                'customer_group', 'product_custom_field1', 'product_custom_field2', 'invoice_schemes'));
    }
    ...
    public function getproductSellGroupedReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if(!empty($request->get('invoice_scheme_id'))){
                 $invoice_scheme_id = $request->get('invoice_scheme_id');
                if($request->invoice_scheme_id !== 'suspended'){
                    $query->where('t.invoice_scheme_id', $invoice_scheme_id)->where('t.is_suspend', 0);
                }else{
                    $query->where('t.is_suspend', 1);
                }
            }else{
   ...
   public function getProfit($by = null)
    {
        ...
        $query = TransactionSellLine::join('products as P', 'transaction_sell_lines.product_id', '=', 'P.id')
         ...
         * 
         * - Sale Price = unit_price + item_tax
         * - Purchase Price:
         *     If sub_unit_id exists and product.sub_unit_prices JSON has price → use it
         *     Else use variation.dpp_inc_tax
         * - If sub_unit has base_unit_multiplier > 1, divide purchase price by multiplier
         */

        $query->select(FacadesDB::raw("
            ROUND(
                SUM(
                    ...
                    COALESCE(
                        CASE 
                            WHEN SU.id IS NOT NULL 
                                AND JSON_CONTAINS_PATH(P.sub_unit_prices, 'one', CONCAT('$.', SU.id)) 
                            THEN 
                                CAST(
                                    JSON_UNQUOTE(
                                        JSON_EXTRACT(P.sub_unit_prices, CONCAT('$.', SU.id))
                                    ) AS DECIMAL(20,2)
                                ) / 
                                CASE 
                                    WHEN SU.base_unit_multiplier > 1 
                                    THEN SU.base_unit_multiplier 
                                    ELSE 1 
                                END
                            ELSE V.dpp_inc_tax
                        END, 0
        ...
        if (!empty(request()->start_date) && !empty(request()->end_date)) {
            $query->whereBetween(DB::raw('DATE(sale.transaction_date)'), [request()->start_date, request()->end_date]);
        }

        // 📊 Grouping logic by filters
        if ($by == 'product') {
            $query
        ...
        if ($by == 'category') {
            $query
            // y->join('variations as V', 'transaction_sell_lines.variation_id', '=', 'V.id')
        ...
        if ($by == 'category') {
            $datatable->editColumn('category', '{{$category ?? __("lang_v1.uncategorized")}}');
        }

        if ($by == 'brand') {
            $datatable->editColumn('brand', '{{$brand ?? __("report.others")}}');
        }
    ...
    public function getStockValue()
    {
     ...
     /**
     * Non-POS Cash Closing (Arqueo de Caja) - view
     */
    public function getNonPosClosing()
    {
        if (! auth()->user()->can('register_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        return view('report.non_pos_register_report')
            ->with(compact('locations'));
    }

    /**
     * Non-POS Cash Closing (Arqueo de Caja) - data
     */
    public function getNonPosClosingData()
    {
        if (! auth()->user()->can('register_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $start_date = ! empty($start_date) ? $start_date : \Carbon::now()->format('Y-m-d');
        $end_date = ! empty($end_date) ? $end_date : $start_date;

        $util = new NonPosRegisterUtil();
        $openings = $util->getOpeningBalances($business_id, $start_date, ! empty($location_id) ? (int) $location_id : null);
        $movements = $util->getDailyMovements($business_id, $start_date, $end_date, ! empty($location_id) ? (int) $location_id : null);

        // Compute totals and final balances
        ...
        if (request()->get('export') === 'xlsx') {
            $file = 'NonPos_Closing_'.str_replace('-', '', $start_date).'_'.str_replace('-', '', $end_date).'.xlsx';
            return \Maatwebsite\Excel\Facades\Excel::download(
                new \App\Exports\NonPosRegisterExport(
                    $business_id,
                    $start_date,
                    $end_date,
        ...
        return [
            'opening' => [
                'caja_menor' => $opening_caja,
                'accounts' => $openings['accounts'] ?? []
            ],
    ...
    /**
     * Open a General Closing (insert cash_registers row with closing_type='general').
     */
    public function openGeneralClosing()
    {
        if (! auth()->user()->can('register_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        // Close any existing open general closing for this business/user/location to avoid duplicates
        $query = \App\CashRegister::where('business_id', $business_id)
            ->where('status', 'open')
            ->where('closing_type', 'general');
        ...
        $cr = \App\CashRegister::create($data);
        return ['status' => 'ok', 'id' => $cr->id];
    }

    /**
     * Close current General Closing (set closed_at and status=close) then return a download URL for export.
     */
    public function closeGeneralClosing()
    {
        if (! auth()->user()->can('register_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($cr) {
            // Compute closing totals for the requested range and persist them on the register
            $start_param = request()->get('start_date');
            $start_for_calc = ! empty($start_param) ? $start_param : $end_date;

            $util = new \App\Utils\NonPosRegisterUtil();
            $openings = $util->getOpeningBalances($business_id, $start_for_calc, ! empty($location_id) ? (int) $location_id : null);
            $movements = $util->getDailyMovements($business_id, $start_for_calc, $end_date, ! empty($location_id) ? (int) $location_id : null);

            $opening_caja = (float) ($openings['caja_menor'] ?? 0);
            $ingresos_total = (float) (($movements['ingresos']['sales'] ?? 0) + ($movements['ingresos']['abonos'] ?? 0));
            $egresos_total = (float) (($movements['egresos']['gastos'] ?? 0) + ($movements['egresos']['anticipos'] ?? 0) + ($movements['egresos']['pagos'] ?? 0));
            $final_caja = $opening_caja + $ingresos_total - $egresos_total;

            // Persist summary
            $summary = [
                'start_date' => $start_for_calc,
                'end_date' => $end_date,
                'opening_petty_cash' => $opening_caja,
                'total_income' => $ingresos_total,
                'total_expense' => $egresos_total,
                'final_petty_cash' => $final_caja,
            ];

            $cr->closing_amount = $final_caja;
            $cr->closing_note = json_encode($summary);
            $cr->status = 'close';
            $cr->closed_at = \Carbon::now();
            $cr->save();
        }

        // Return URL for export. Caller will navigate to it.
        $start_date = request()->get('start_date') ?: $end_date;
        $exportUrl = url('/reports/non-pos-closing/data?'.http_build_query([
        ...
        return ['status' => 'ok', 'export_url' => $exportUrl];
    }
