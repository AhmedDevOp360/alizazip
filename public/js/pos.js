...
function checkCostPriceValidation(tr) {
  ...
  var unitPriceInput = tr.find(
    "input.pos_unit_price_inc_tax, input.pos_unit_price"
  );
...
function updateProductQtyBadge(variationId) {
  ...
  var enableStock =
    box.data("enable-stock") == 1 || box.data("enable-stock") === "1";
  ...
  var soldQty = getCartQtyForVariation(
    variationId,
...
function updateSelectedQtyBadge(variationId) {
  var selected = getCartQtyForVariation(
    variationId,
    $("input#location_id").val()
  );
  var box = $('.product_box[data-variation_id="' + variationId + '"]');
...
$(document).ready(function () {
  ...
  $("select#select_location_id").change(function () {
    ...
    if (default_price_group) {
      if (
        $("#price_group option[value='" + default_price_group + "']").length > 0
      ) {
    ...
    function () {
      ...
      setTimeout(function () {
        ...
        try {
          var variationIdForRow = tr.find(".row_variation_id").val();
          if (
            typeof updateProductQtyBadge === "function" &&
            variationIdForRow
          ) {
            updateProductQtyBadge(variationIdForRow);
          }
  ...
  $("table#pos_table").on("change", "select.sub_unit", function () {
    ...
    if (!selected_option.val()) {
      ...
      if (cost_price) {
        tr.find("input.pos_unit_price, input.pos_unit_price_inc_tax").each(
          function () {
...
function pos_product_row(
  ...
  if (add_via_ajax) {
    ...
    var insertAtTop =
      typeof should_prepend_pos_row !== "undefined" && should_prepend_pos_row;
    should_prepend_pos_row = true;
...
$(document).on("click", "#send_for_sercice_staff_replacement", function (e) {
  ...
  if (invoice_no) {
    $.ajax({
      method: "get",
      url:
        /validate-invoice-to-service-staff-replacement/ + encodeURI(invoice_no),
      dataType: "json",
      success: function (result) {
        if (result.success == true) {
...
function saveFormDataToLocalStorage() {
  ...
  let shippingIndex = formArray.findIndex(
    (item) => item.name === "shipping_charges_amount"
  );
  if (shippingIndex !== -1) {
    // If exists, update the value
    formArray[shippingIndex].value = $("#shipping_charges_amount")
      .text()
      .trim();
  } else {
    // If not exists, push new entry
  ...
  let totalPayingIndex = formArray.findIndex(
    (item) => item.name === "total_paying_input"
  );
  ...
  let changeReturnIndex = formArray.findIndex(
    (item) => item.name === "change_return"
  );
  ...
  let balanceDueIndex = formArray.findIndex(
    (item) => item.name === "in_balance_due"
  );
