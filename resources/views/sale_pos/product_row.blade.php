...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	...
	<td>
		...
		<div title="@lang('lang_v1.pos_edit_product_price_help')" style="display: inline; font-size: 16px">
		<span class="text-link text-dark cursor-pointer" data-toggle="modal" data-target="#row_edit_product_price_modal_{{$row_count}}">
			{!! $product_name !!}
		</span>
		</div>
		@else
			<span class="pos-location-indicator"></span>
			{!! $product_name !!}
		@endif
...
<img src="@if(count($product->media) > 0)
		...
		<input type="hidden" name="products[{{$row_count}}][line_location_id]" class="line_location_id" value="{{ $transactionLocation->id ?? '' }}">
		<input type="hidden" 
			class="product_type" 
	...
	<td>
		...
		<input type="text" data-min="1" style="width: 6vh; padding: 0px 2px 0px 4px"
		...
		<i class="glyphicon glyphicon-trash text-danger pos_remove_row cursor-pointer" aria-hidden="true"  style="font-size:25px"></i>
		...
		@if(count($sub_units) > 0)
            ...
            <select name="products[{{$row_count}}][sub_unit_id]" class="form-control input-sm sub_unit">
                <option value="" data-multiplier="1" data-unit_name="" data-allow_decimal="1">Unidad</option>
                @foreach($sub_units as $key => $value)
    ...
    <td class="text-center">
		...
		@endphp
		<input style="width: 9vh" type="{{$subtotal_type}}" class="form-control pos_line_total @if(!empty($pos_settings['is_pos_subtotal_editable'])) input_number @endif" value="{{@num_format($product->quantity_ordered*$unit_price_inc_tax )}}">
